...
int main(int argc, char *argv[]) {
    ...
    cout << "C_1_ssp_precomputed_randomness starts" << endl;
    ...
    //in the following, we create some precomputed randomness to test ssp_precomputed
    paillier_plaintext_t *r_a = (paillier_plaintext_t *) malloc(sizeof(paillier_plaintext_t));
    paillier_plaintext_t *negative_two_times_r_a = (paillier_plaintext_t *) malloc(sizeof(paillier_plaintext_t));
    paillier_plaintext_t *negative_r_a_squared = (paillier_plaintext_t *) malloc(sizeof(paillier_plaintext_t));

    paillier_ciphertext_t *enc_r_a = paillier_create_enc_zero();
    paillier_ciphertext_t *enc_negative_r_a_squared = paillier_create_enc_zero();

    mpz_init(r_a->m);
    mpz_init(negative_two_times_r_a->m);
    mpz_init(negative_r_a_squared->m);

    generate_random_plaintext(r_a, pk, paillier_get_rand_devurandom);

    mpz_mul_ui(negative_two_times_r_a->m, r_a->m, 2);
    mpz_mod(negative_two_times_r_a->m, negative_two_times_r_a->m, pk->n);
    mpz_sub(negative_two_times_r_a->m, pk->n, negative_two_times_r_a->m);

    mpz_mul(negative_r_a_squared->m, r_a->m, r_a->m);
    mpz_mod(negative_r_a_squared->m, negative_r_a_squared->m, pk->n);
    mpz_sub(negative_r_a_squared->m, pk->n, negative_r_a_squared->m);

    paillier_enc(enc_r_a, pk, r_a, paillier_get_rand_devurandom);
    paillier_enc(enc_negative_r_a_squared, pk, negative_r_a_squared, paillier_get_rand_devurandom);
    //in the above, we create some precomputed randomness to test ssp_precomputed

    //initialize enc_a_times_b for ssp protocol
    paillier_ciphertext_t *enc_a_squared = paillier_create_enc_zero();
    ...
    while (1) {
        ...
        if (command_to_receive == SMP) {
            ...
            for (i = 0; i < num_smp; i = i + 1) {
                ...
                execute_ssp_precomputed_randomness_C_1(enc_a_squared, enc_a, pk, sockfd_with_C_2, negative_two_times_r_a, enc_r_a, enc_negative_r_a_squared);

#ifdef DEBUG_SMP
                //send enc_a_squared to C_2 and let C_2 check if the result is correct
                socket_send_paillier_ciphertext_t(sockfd_with_C_2, enc_a_squared);
#endif
    ...
    paillier_freeplaintext(negative_two_times_r_a);

    paillier_freeciphertext(enc_r_a);
    paillier_freeciphertext(enc_negative_r_a_squared);
