...
#ifdef DEBUG_SOCKET_SEND_PAILLIER_CIPHERTEXTS
    timeval before_export;
    gettimeofday(&before_export, NULL);
    printf("before_export: %ld\n", (before_export.tv_sec * 1000000 + before_export.tv_usec));
#endif
...
#ifdef DEBUG_SOCKET_SEND_PAILLIER_CIPHERTEXTS
    timeval after_export;
    gettimeofday(&after_export, NULL);
    printf("after_export: %ld\n", (after_export.tv_sec * 1000000 + after_export.tv_usec));
#endif

    int num_bytes_to_send = in_num_ciphertexts * 256;
    int num_bytes_sent_so_far = 0;
    int current_num_bytes_sent = 0;
    int num_bytes_to_send_each_time = 40960000;
    int num_bytes_left = num_bytes_to_send;
    while (num_bytes_sent_so_far < num_bytes_to_send) {

        if (num_bytes_to_send_each_time > num_bytes_left) {
            current_num_bytes_sent = send(sockid, out_buff + num_bytes_sent_so_far,
                                          num_bytes_left, 0);
        } else { //num_bytes_to_send_each_time <= num_bytes_left
            current_num_bytes_sent = send(sockid, out_buff + num_bytes_sent_so_far,
                                          num_bytes_to_send_each_time, 0);
        }

        num_bytes_sent_so_far = num_bytes_sent_so_far + current_num_bytes_sent;
        num_bytes_left = num_bytes_left - current_num_bytes_sent;
...
int socket_receive_paillier_ciphertexts(paillier_ciphertext_t ***out_ct_s, int in_num_ciphertexts, int sockid) {
    ...
    int num_bytes_to_receive = in_num_ciphertexts * 256;
    int num_bytes_received_so_far = 0;
    int current_num_bytes_received = 0;
    int num_bytes_to_receive_each_time = 40960000;
    int num_bytes_left = num_bytes_to_receive;
    while (num_bytes_received_so_far < num_bytes_to_receive) {

        if (num_bytes_left > num_bytes_to_receive_each_time) {
            current_num_bytes_received = recv(sockid, in_buff + num_bytes_received_so_far,
                                              num_bytes_to_receive_each_time, 0);
        } else { //num_bytes_left <= num_bytes_to_receive_each_time
            current_num_bytes_received = recv(sockid, in_buff + num_bytes_received_so_far,
                                              num_bytes_left, 0);
        }
        ...
        num_bytes_left = num_bytes_left - current_num_bytes_received;
    }
