...
int main(int argc, char *argv[]) {
    ...
    cout << "C_1_smp_pipelined starts" << endl;
    ...
    //in the following, we create some precomputed randomness to test smp_pipelined
    ...
    paillier_ciphertext_t **enc_a_s = initialize_paillier_ciphertexts_1_d(num_smp);
    paillier_ciphertext_t **enc_b_s = initialize_paillier_ciphertexts_1_d(num_smp);

    paillier_plaintext_t **negative_r_a_s = initialize_paillier_plaintexts_1_d(num_smp);
    paillier_plaintext_t **negative_r_b_s = initialize_paillier_plaintexts_1_d(num_smp);

    paillier_ciphertext_t **enc_r_a_s = initialize_paillier_ciphertexts_1_d(num_smp);
    paillier_ciphertext_t **enc_r_b_s = initialize_paillier_ciphertexts_1_d(num_smp);
    paillier_ciphertext_t **enc_negative_r_a_times_r_b_s = initialize_paillier_ciphertexts_1_d(num_smp);

    for (i = 0; i < num_smp; i = i + 1) {
        mpz_set(enc_a_s[i]->c, enc_a->c);
        mpz_set(enc_b_s[i]->c, enc_b->c);

        mpz_set(enc_r_a_s[i]->c, enc_r_a->c);
        mpz_set(enc_r_b_s[i]->c, enc_r_b->c);

        mpz_set(negative_r_a_s[i]->m, negative_r_a->m);
        mpz_set(negative_r_b_s[i]->m, negative_r_b->m);

        mpz_set(enc_r_a_s[i]->c, enc_r_a->c);
        mpz_set(enc_r_b_s[i]->c, enc_r_b->c);
        mpz_set(enc_negative_r_a_times_r_b_s[i]->c, enc_negative_r_a_times_r_b->c);
    }
    //in the above, we create some precomputed randomness to test smp_pipelined

    //initialize enc_a_times_b for smp protocol
    paillier_ciphertext_t *enc_a_times_b = paillier_create_enc_zero(); //might not be useful later
...
//            }
           ...
           // gmp_printf("enc_a_times_b: %Zd\n", enc_a_times_b->c);

            execute_smp_pipelined_C_1(enc_a_times_b_s,
                                      enc_a_s, enc_b_s,
                                      pk, sockfd_with_C_2,
                                      negative_r_a_s, negative_r_b_s,
                                      enc_r_a_s, enc_r_b_s,
                                      enc_negative_r_a_times_r_b_s,
                                      num_smp);
...
#ifdef DEBUG_SMP
            for (i = 0; i < num_smp; i = i + 1) {
                socket_send_paillier_ciphertext_t(sockfd_with_C_2, enc_a_times_b_s[i]);

                //gmp_printf("enc_a_times_b_s[%ld]: %Zd\n", i, enc_a_times_b_s[i]->c);
            }
#endif
