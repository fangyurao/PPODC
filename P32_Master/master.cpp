...
//void check_clients(struct pool *p, int *in_task_is_done, int *in_spci_is_done);
...
int check_termination_condition(int in_num_iterations, struct pool *p);
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (all_tasks_in_current_iteration_are_done == 1) {
            ...
            //if num_pairs_of_servers > 1, need to send dummy setc information to pair 0
            //...
            if (num_pairs_of_servers > 1) {
                socket_send_command_master_to_C_1(p.clientfd[p.index_of_pair_zero], RECEIVE_SETC_INFO);
                for (i = 0; i < num_dummy_setc_elements; i = i + 1) {
                    socket_send_paillier_ciphertext_t(p.clientfd[p.index_of_pair_zero], enc_dummy_setc_element_s[i]);
                }
                printf("done with sending enc dummy setc elements to pair 0\n");
            }

            //check if we can really leave the while-loop by an invocation to SETC
            termination_condition_satisfied = check_termination_condition(num_iterations, &p);
            ...
            if (termination_condition_satisfied == 1) {
                ...
                for (i = 0; i < num_pairs_of_servers; i = i + 1) {
                    ...
                    if (i == p.index_of_pair_zero) {
                        socket_send_command_master_to_C_1(p.clientfd[i], SPCI);
                        p.previous_assigned_task[i] = DOING_SPCI;
                    } else {
                        socket_send_command_master_to_C_1(p.clientfd[i], WAIT_FOR_FURTHER_INSTRUCTION);
...
void check_clients(paillier_ciphertext_t **out_enc_dummy_received_setc_element_s, 
    ...
    //in the following, we declare and define variables needed for receiving setc information from other pairs
//  int num_dummy_setc_elements = 100;
//
//  paillier_ciphertext_t **enc_dummy_received_setc_element_s =
//      initialize_paillier_ciphertexts_1_d(num_dummy_setc_elements);
    //in the above, we declare and define variables needed for receiving setc information from other pairs
        ...
        if ((conn_fd > 0) && (FD_ISSET(conn_fd, &p->ready_set))) {
            ...
            if (n > 0) {
                ...
                if (request_to_receive == REQUEST_FOR_TASK) {
                    ...
                    //depending on the status of spci_is_done
                    //(i) if spci_is_done == 0
                    // if p->client_id[i] == 0, ask this client to perform spci
                    // else ask this client to wait for further instruction
                    if (*in_spci_is_done == 0) {
                        if (p->client_id[i] == 0) {

                            p->index_of_pair_zero = i;
                            //store the information so that when calling SETC we know which pair to invoke

                            socket_send_command_master_to_C_1(conn_fd, SPCI);
                            p->previous_assigned_task[i] = DOING_SPCI;
                            continue;
                        } else { //not pair 0, but spci_is_done is false
                    ...
                    //if we reach here, then it means currently spci_is_done == 1
                    socket_send_command_master_to_C_1(conn_fd, RECEIVE_CLUSTER_INFO);
                    for (s = 0; s < num_dummy_cluster_elements; s = s + 1) {
                        socket_send_paillier_ciphertext_t(conn_fd, enc_dummy_received_cluster_element_s[s]);
                    }
                    printf("done with sending enc dummy cluster elements to pair %d\n", p->client_id[i]);

                    socket_send_command_master_to_C_1(conn_fd, SSED_OP);
                    p->previous_assigned_task[i] = DOING_SSED_OP;
                    continue;
                ...
                if (request_to_receive == TASK_IS_DONE) {
                    ...
                    if (p->previous_assigned_task[i] == DOING_SPCI) {

                        *in_spci_is_done = 1; //update the status of in_spci_is_done

                        //then receive the necessary cluster information from the C_1 of pair_0
                        //...
                        for (s = 0; s < num_dummy_cluster_elements; s = s + 1) {
                            socket_receive_paillier_ciphertext_t(conn_fd, &(enc_dummy_received_cluster_element_s[s]));
                        }
                        printf("done receiving enc dummy cluster elements from pair 0\n");

                        socket_send_command_master_to_C_1(conn_fd, SSED_OP);
                        p->previous_assigned_task[i] = DOING_SSED_OP;

                        //do not forget to "wake up" those waiting clients/pairs
                        //also need to send the necessary cluster information to other pairs
#ifdef DEBUG_CHECK_CLIENTS
                        printf("try to see if there are waiting clients/pairs\n");
#endif
                        for (j = 0; j <= p->maxi; j++) {
                            if ((p->clientfd[j] != -1) && (p->previous_assigned_task[j] == WAITING)) {

                                socket_send_command_master_to_C_1(p->clientfd[j], RECEIVE_CLUSTER_INFO);

                                //then send the cluster information just received to the corresponding C_1
                                //...
                                for (s = 0; s < num_dummy_cluster_elements; s = s + 1) {
                                    socket_send_paillier_ciphertext_t(p->clientfd[j], enc_dummy_received_cluster_element_s[s]);
                                }
                                printf("done with sending enc dummy cluster elements to pair %d\n", j);

                                socket_send_command_master_to_C_1(p->clientfd[j], SSED_OP);
                                p->previous_assigned_task[j] = DOING_SSED_OP;
#ifdef DEBUG_CHECK_CLIENTS
                                printf("wake up client %d \n", p->client_id[j]);
#endif
                            }
                        }

                        continue;
                        //could consider to "return" instead of "continue" here
                        //because if we have received "TASK_IS_DONE" by pair 0
                        //any other pair should be waiting
                        //i.e., no need to check for other possible descriptors
                    }

                    if (p->previous_assigned_task[i] == DOING_SSED_OP) {
                    ...
                    if (p->previous_assigned_task[i] == DOING_SBD) {
                        ...
                        socket_send_command_master_to_C_1(conn_fd, SMIN_K);
                        p->previous_assigned_task[i] = DOING_SMIN_K;
                        continue;

                    }

                    if (p->previous_assigned_task[i] == DOING_SMIN_K) {

                        socket_send_command_master_to_C_1(conn_fd, SMP);
                        p->previous_assigned_task[i] = DOING_SMP;
                        continue;

                    }

                    if (p->previous_assigned_task[i] == DOING_SMP) {

                        //if this is coming from the pair 0
                        //do nothing, put pair 0 to wait
                        //else (not pair 0)
                        //receive SETC related information
                        //...
                        if (p->client_id[i] == 0) {//this C_1 is pair 0



                        } else {//this C_1 is not pair 0

                            //Note by Fang-Yu on 2015/02/13, enc_dummy_received_setc_element should be 
                            //accessible to the outside...
                            //start from here...
                            for (s = 0; s < in_num_dummy_setc_elements; s = s + 1) {
                                socket_receive_paillier_ciphertext_t(conn_fd, &(out_enc_dummy_received_setc_element_s[s]));
                            }
                            printf("done with receiving enc dummy setc elements from client %d\n", p->client_id[i]);
...
int check_termination_condition(int in_num_iterations, struct pool *p) {
    ...
    socket_send_command_master_to_C_1(fd_pair_zero, SETC);

    n = socket_receive_request_C_1_to_master(fd_pair_zero, &request_to_receive);

    if (n > 0) {

        if (request_to_receive == TASK_IS_DONE) {

            printf("designated C_1 is done with SETC\n");
        ...
        if (n == 0) {
            ...
            //connection closed
#ifdef DEBUG_CHECK_TERMINATION_CONDITION
            printf("select_server: socket %d hung up\n", fd_pair_zero);
#endif
        ...
        /* EOF detected, remove descriptor from pool */
        close(fd_pair_zero);
        FD_CLR(fd_pair_zero, &p->read_set);
        p->clientfd[p->index_of_pair_zero] = -1;
        p->client_id[p->index_of_pair_zero] = -1;
        p->num_connected_clients = p->num_connected_clients - 1;
        p->previous_assigned_task[p->index_of_pair_zero] = -1;

    }

    if (in_num_iterations < 1) {
        return 0;
    } else {
        return 1;
    }
