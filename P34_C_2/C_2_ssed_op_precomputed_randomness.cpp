...
int main(int argc, char *argv[]) {
    cout << "C_2_ssed_op_precomputed_randomness starts" << endl;
    ...
    //in the following, we prepare some precomputed randomness needed in ssed_op_precomputed_randomness
    paillier_plaintext_t *zero = (paillier_plaintext_t *)malloc(sizeof(paillier_plaintext_t));
    mpz_init(zero->m);
    mpz_set_ui(zero->m, 0);

    paillier_ciphertext_t **enc_zero_s_smp = initialize_paillier_ciphertexts_1_d(num_dimensions_ssed_op);
    paillier_ciphertext_t **enc_zero_s_ssp = initialize_paillier_ciphertexts_1_d(num_dimensions_ssed_op);

    for (i = 0; i < num_dimensions_ssed_op; i = i + 1) {
        paillier_enc(enc_zero_s_smp[i], pk, zero, paillier_get_rand_devurandom);
        paillier_enc(enc_zero_s_ssp[i], pk, zero, paillier_get_rand_devurandom);
    }
    //in the above, we prepare some precomputed randomness needed in ssed_op_precomputed_randomness
    ...
    while (1) {
        ...
        if (command_to_receive == SSED_OP) {
            ...
            for (i = 0; i < num_ssed_op; i = i + 1) {
                ...
                execute_ssed_op_precomputed_randomness_C_2(sockfd_with_C_1, pk, 
                         sk, num_dimensions_ssed_op, 
                         enc_zero_s_smp, 
                         enc_zero_s_ssp);
    ...
    free_paillier_ciphertexts_1_d(num_dimensions_ssed_op, enc_zero_s_smp);
    free_paillier_ciphertexts_1_d(num_dimensions_ssed_op, enc_zero_s_ssp);
