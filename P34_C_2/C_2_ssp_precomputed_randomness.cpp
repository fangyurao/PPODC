...
int main(int argc, char *argv[]) {
    cout << "C_2_ssp_precomputed_randomness starts" << endl;
    ...
    paillier_plaintext_t *a_squared = 0;
    paillier_ciphertext_t *enc_a_squared = 0;

    //create an encryption of zero to test execute_precomputed_ssp
    ...
    while (1) {
        ...
        if (command_to_receive == SMP) {
            ...
            for (i = 0; i < num_smp; i = i + 1) {
                ...
                execute_ssp_precomputed_randomness_C_2(sockfd_with_C_1, pk, sk, enc_zero);

#ifdef DEBUG_SMP
                //try to receive the result from the corresponding C_1 to check if the result is correct
                socket_receive_paillier_ciphertext_t(sockfd_with_C_1, &enc_a_squared);


                a_squared = paillier_dec(a_squared, pk, sk, enc_a_squared);
                gmp_printf("a_squared[%d]: %Zd\n", i, a_squared->m);
#endif
    ...
    //clean-up
#ifdef DEBUG_SMP
    paillier_freeplaintext(a_squared);
    paillier_freeciphertext(enc_a_squared);
#endif
